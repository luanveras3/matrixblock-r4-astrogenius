...
bool MatrixColorV3::begin() {
  ...
  // Configure exposure BEFORE enabling the ADC. Without this the chip keeps
  // its post-reset defaults — minimum integration time and 1x gain — and in
  // ordinary room light the clear channel lands under the `c < 20` floor that
  // getR/getG/getB use to mean "not enough light". Every read then returns 0
  // and getColorID() returns -1, while the sensor answers on I2C and looks
  // perfectly healthy. It is simply underexposed. TCS34725_ATIME and
  // TCS34725_CONTROL were already defined in the header and never written,
  // which is what gave the omission away.
  writeRegister(TCS34725_ATIME, TCS34725_INTEGRATIONTIME_50MS);
  writeRegister(TCS34725_CONTROL, 0x01);   // AGAIN = 4x (no constant defined)

  writeRegister(TCS34725_ENABLE, TCS34725_ENABLE_PON);
  delay(3);
  writeRegister(TCS34725_ENABLE, TCS34725_ENABLE_PON | TCS34725_ENABLE_AEN);
  // One full integration cycle must elapse before the first conversion is
  // valid; without it the first read still sees zeros.
  delay(60);

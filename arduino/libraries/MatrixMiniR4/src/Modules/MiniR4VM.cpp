...
// Transport coupling now goes through a static YieldFn (set by whichever
// runtime is active); the VM no longer includes any transport header.
// Previous branches included MiniR4BLERuntime.h here to call pollBleOnly()
// during DELAY_MS slices; the callback keeps the same behavior without
// hardwiring the VM to a specific transport.

namespace {
    MiniR4VM::YieldFn s_yieldFn = nullptr;
}

void MiniR4VM::setYieldCallback(YieldFn fn) { s_yieldFn = fn; }
